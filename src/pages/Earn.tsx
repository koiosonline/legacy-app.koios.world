  ...
  return (
    <>
      ...
      <div className={"selector-container"}>
        <button
          onClick={() => handleActiveView(true, false)}
          className={vacanciesActive ? "button button--active" : "button"}
        >
          Take on Challenge
        </button>
        <button
          onClick={() => handleActiveView(false, true)}
          className={submissionsActive ? "button button--active" : "button"}
        >
          Submit a Challenge
        </button>
        <Link to={"coming-soon"} className={"button"}>
          Paid micro challenges
        </Link>
      </div>
          ...
          <div className={"categories"}>
            {earnData.categories.map((button, index) => {
              return (
                <button
                  className={
                    activeCat.includes(button.title)
        ...
        <div className={"partner"}>
          <div className={"partner__header"}>
            ...
            <p className={"sub-title"}>
              {partnerData.description} <span>&#x1F680;</span>
            </p>
          </div>
          <div>
